...
> - **Beta-test approach decided (2026-04-28):** Round 1 = 10 unmoderated UserTesting.com sessions, mixed Scripts A (core install + first rule, 6 sessions) and B (power user / SMS, 4 sessions). Cost ~$490, 1–2 weeks turnaround, runs in parallel with the OAuth-verification wait. No code changes (no per-tester Pro comp; one-shot sessions don't need persistent Pro access). Top-level workspace lives at `usertesting/` — see `usertesting/README.md` for layout (`docs/` for canonical scripts, `outgoing/` for filled per-round scripts (gitignored), `incoming/` for downloaded recordings (gitignored), `findings/` for triage docs (committed)). Replaces the originally-considered personal-network beta — the developer doesn't have a qualified-tester network. Founding Member tier remains a separate launch concern, unrelated to the UserTesting round.
> - **Round 1 scope reduced (2026-04-29):** Script B (SMS path) and "Start scheduled scans" / auto-trigger flows are deferred from Round 1. Round 1 is now **all 10 sessions on Script A** (install + first rule + Calendar alert + manual `Scan email now`). SMS path deferred because provider setup (Twilio A2P 10DLC, Telnyx number provisioning) takes days that don't fit a 20-min session, and Textbelt's free tier rate-limits at 1 SMS/day per IP. Auto-trigger flow deferred because the 60-min trigger floor means testers can't observe an automatic fire during a 20-min session, and leaving scheduled scans on after sessions end would burn the $5 sandbox Gemini budget. Manual `Scan email now` exercises the same code path as auto scans (just with `force: true`), so usability of the result card is fully covered. `usertesting/docs/script_b_power.md` keeps a deferred banner so a future round can revive it.

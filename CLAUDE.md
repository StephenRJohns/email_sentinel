...
**"Scan email now" universal action runs immediately and returns the result card:** `actionRunCheckNow` calls `runMailCheck({force:true})` synchronously and returns `buildScanResultCard_()` via `universalCardResponse_`. The platform shows its own loading state on the side panel while the universal-action handler runs — there is no button-attached spinner (universal-action responses have no button to attach one to), but the panel-level loading indicator is sufficient feedback for the 10–60 second scan. An earlier iteration added an intermediate `buildPreScanCard_()` to expose a button-attached spinner, but the platform feedback turned out to be adequate, so the extra tap was removed. `handleRunCheckNow` (used by the home card's "Scan email now" button) follows the same logic but returns an `ActionResponseBuilder` navigation push instead of a `UniversalActionResponseBuilder`, and gets the regular button-attached spinner since it is a button click.

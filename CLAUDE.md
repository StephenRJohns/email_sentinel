...
**"Scan email now" universal action runs immediately and returns the result card:** `actionRunCheckNow` calls `runMailCheck({force:true})` synchronously and returns `buildScanResultCard_()` via `universalCardResponse_`. There is no intermediate card. The platform shows no spinner during the scan (universal-action responses have no button to attach one to), so the UI will appear briefly unresponsive — accepted tradeoff for eliminating the extra tap. `handleRunCheckNow` (used by the home card's "Scan email now" button) follows the same logic but returns an `ActionResponseBuilder` navigation push instead of a `UniversalActionResponseBuilder`.

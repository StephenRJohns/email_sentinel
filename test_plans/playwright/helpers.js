...
// Gmail add-on iframes are served from script.googleusercontent.com
const ADDON_IFRAME = 'iframe[src*="script.googleusercontent.com"]';
...
async function expectToast(page, text, timeout = 15_000) {
  // Gmail toasts can appear as aria-live regions or specific notification divs
  const toast = page.locator(`[role="alert"]:has-text("${text}"), [aria-live]:has-text("${text}")`).first();
  await expect(toast).toBeVisible({ timeout });
}

// Click a Card button by its visible label
...
async function waitForEmailInInbox(page, subject, retries = 6) {
  for (let i = 0; i < retries; i++) {
    await page.goto(`${GMAIL_URL}/#search/subject:${encodeURIComponent(subject)}`, { waitUntil: 'networkidle' });
